...
int main(int argc, char* argv[]) {
        ...
        unsigned long long int block_size = 4096 * mul;
        ...
        if(!id) {
                global_count++;
                printf("The total number of prime:%llu, total time:%10.6f, total node%s, with cache size: %llu.\n", global_count, elapse_time, argv[2], block_size);
        }
        MPI_Finalize();
        return 0;
}
