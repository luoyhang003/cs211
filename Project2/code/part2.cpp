...
void mydgetrf_blockversion_optimized(double* a, int* pvt, double* tempv, int N, int block);
...
int main(int argc, char** argv) {
        ...
        int *pvt1 = (int*)calloc(sizeof(int), N);
        int *pvt2 = (int*)calloc(sizeof(int), N);

        double* tempv1 = (double*)calloc(sizeof(double), N);
        double* tempv2 = (double*)calloc(sizeof(double), N);
        ...
        mydgetrf_blockversion(a1, pvt1, tempv1, N, block_size);
        ...
        clock_gettime(CLOCK_MONOTONIC, &start);
        mydgetrf_blockversion_optimized(a2, pvt2, tempv2, N, block_size);
        clock_gettime(CLOCK_MONOTONIC, &end);
...
void mydgetrf_blockversion_optimized(double* a, int* pvt, double* tempv,
                                     int N, int block) {
        int maxind, end, temp;
        double max;

        int block_i, i, t, k, j, l, m;

        double *mm;

        for(block_i=0; block_i<N; block_i+=block) {
                end = block_i+block-1;
                for(i=block_i; i<=end; i++) {
                        maxind = i;
                        max = fabs(a[i*N+i]);
                        for(t=i+1; t<N; t++) {
                                if(fabs(a[t*N+i])>max) {
                        ...
                        if(max==0.0) {
                                ...
                                if(maxind != i) {
                                        ...
                                        *((unsigned*)(&pvt[i])) = *((unsigned*)(&pvt[i])) ^ *((unsigned*)(&pvt[maxind]));
                                        *((unsigned*)(&pvt[maxind])) = *((unsigned*)(&pvt[i])) ^ *((unsigned*)(&pvt[maxind]));
                                        *((unsigned*)(&pvt[i])) = *((unsigned*)(&pvt[i])) ^ *((unsigned*)(&pvt[maxind]));
                                        ...
                                        for(k=0; k<N; k++) {
                                                ...
                                                *((unsigned*)(&a[i*N+k])) = *((unsigned*)(&a[i*N+k])) ^ *((unsigned*)(&a[maxind*N+k]));
                                                *((unsigned*)(&a[maxind*N+k])) = *((unsigned*)(&a[i*N+k])) ^ *((unsigned*)(&a[maxind*N+k]));
                                                *((unsigned*)(&a[i*N+k])) = *((unsigned*)(&a[i*N+k])) ^ *((unsigned*)(&a[maxind*N+k]));
                                        }
                                }
                        }
                        ...
                        for(j=i+1; j<N; j++) {
                                register double aa = a[j*N+i]/a[i*N+i];
                                for(k=i+1; k<=end; k++) {
                                        a[j*N+k] = a[j*N+k] - aa * a[i*N+k];
                                }
                        }
                }
                ...
                for(l = block_i; l<=end; l++) {
                        for(m=block_i; m<=end; m++) {
                                if(l>m) {
